...
def log_start(log_name):
    ...
    if not os.path.exists('paperlog_fufa/' + log_name):
        os.mkdir('paperlog_fufa/' + log_name)

    with open('paperlog_fufa/' + log_name + '/python_file.py', 'a', encoding='utf-8') as f:
        with open(os.path.basename(__file__), 'r', encoding='utf-8') as f2:
            file = f2.read()
        f.write(file)
    path = 'paperlog_fufa/' + log_name
...
def main(name, pathd, time_c, path):
    ...
    sentiment_train_set = SentimentDataset('sentiment/af/inhp_train_sap_newend_1.csv')
    sentiment_train_loader = DataLoader(sentiment_train_set, batch_size=batch_size, shuffle=True, num_workers=0)
    #加载验证集
    sentiment_valid_set = SentimentDataset('sentiment/af/inhp_valid_sap_newend_1.csv')
    ...
    sentiment_test_set = SentimentDataset('sentiment/af/inhp_test_sap_newend_1.csv')
    sentiment_test_loader = DataLoader(sentiment_test_set, batch_size=batch_size, shuffle=False, num_workers=0)
    # 加载外部验证集
    entiment_outvalid_set = SentimentDataset('sentiment/af/inhp_outvalid_sap_newend_1.csv')
            ...
            for batch in dataloader:
                ...
                if i % 400 == 0:  # 400/4=100
                    ...
                    if roc_auc > best_score:
                        best_score = roc_auc
                        result.to_csv('result/sap/pres/{}_result_valid.csv'.format(name), index=False)
                        state = {'net': model.state_dict(),  'epoch': epochs,
                                 'steps': i}
                        modelpath = '/data/LM_code/ChineseStrokeBert/StrokeTask/finetuningSAP/PRES/baseline_{}.pth'.format(name)
    ...
    log2 = "test loss: {},valid acc:{},f1:{},roc_auc:{},pr_auc:{}".format(valid_loss, valid_acc, f1, roc_auc,pr_auc)
    print(log2)
    log(log2, path)
    ...
    resultinvalid.to_csv('result/sap/pres/{}_result_invalid.csv'.format(name), index=False)
    resultoutvalid.to_csv('result/sap/pres/{}_result_outvalid.csv'.format(name),index=False)
...
if __name__ == '__main__':
    for name in ["macbertend17","macbertend17cnn","macbertend17lstm","macbertend17RCNN","macbertend17FGM",
                 "macbertend17cnnFGM","macbertend17lstmFGM","macbertend17RCNNFGM"]:

        #macbertend3 inhop"macbertend7","macbertend7cnn","macbertend7lstm"
        #macbertend4 SAP_2
        #macbertend5 SAP_3
        #macbertend6 inhpsap1
        #macbertend7cnnFGM inhpsap2
        #macbertend8 inhpsap1new
        #macbertend8cnnFGM2 epsilon=0.5
        #macbertend8cnnFGM3 epsilon=0.8
        #macbertend8cnnFGM4 epsilon=1
        #macbertend9 new2 inhpsap
        #macbertend10 new2 inhpsap2
        #macbertend11
        #macbertend12
        #macbertend13
